...
def run_mcmc(
    ...
    *,
    ...
    """Sample the posterior using :mod:`emcee` for 3D hyper-parameters.
    ...
    if initial_guess is None:
        # Default starting point for 3D η = (alpha_sps, mu_h, mu_gamma)
    ...
    print("[INFO] 从头开始采样 (3D)")
    # Initialize walkers around the initial guess in 3D
    # Ensure non-zero perturbations in all dimensions and full-rank variation
    eps = 1e-2
    noise = eps * np.random.randn(nwalkers, ndim)
    # Add tiny axis-aligned offsets to first 'ndim' walkers to guarantee spanning directions
