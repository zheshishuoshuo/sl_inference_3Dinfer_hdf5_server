...
def main(
    ...
    *,
    ...
    n_galaxy = int(n_galaxy) if n_galaxy is not None else 150000
    ...
    df_lens, mock_lens_data, mock_observed_data, samples_dict = run_mock_simulation(
        n_galaxy,
          logalpha=logalpha, seed=seed, nbkg=4e-4, if_source=True,process = 1-
    )
    ...
    backend_file = f"chains3D_{int(mock_lens_data.shape[0])}lens_noeta_{ts}_{seed}.h5"
    ...
    sampler = run_mcmc(
        ...
        nsteps=nsteps,
        nwalkers=40,
        # 3D initial guess: (alpha_sps, mu_h, mu_gamma)
        initial_guess=np.array([0.0, 12.9, 1.0]),
    ...
    # Override names for 3D version
    # param_names = [r"$\\alpha_{sps}$", r"$\\mu_h$", r"$\\mu_\\gamma$"]
    param_names = [r"$\alpha_{sps}$", r"$\mu_h$", r"$\mu_\gamma$"]
    ...
    # 真值
    # Ground-truth for reference (3D): η = (alpha_sps, mu_h, mu_gamma)
    true_values = [logalpha, model_p["mu_h0"], 1.0]
    ...
    try:
        ...
        write_run_hdf5(
            ...
            n_galaxy=n_galaxy,
            eta=eta,
            # Save true values for 3D parameters (η)
            true_values=[logalpha, model_p["mu_h0"], 1.0],
            seed=seed,
            git_root=base_dir,
            samples_dict={
